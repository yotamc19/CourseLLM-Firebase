...
class QuizGenerator(dspy.Module):
    ...
    def forward(
        ...
        return self.generate_quiz(
            ...
            num_questions=str(num_questions)  # Convert to string for DSPy signature
        )
...
class PersonalizedLearningAssistant(dspy.Module):
    ...
    def summarize_materials(self, materials: List[str]) -> Dict:
        ...
        # DSPy may return key_points as a string, convert to list if needed
        key_points = result.key_points
        if isinstance(key_points, str):
            # Try to parse as a list from string representation
            key_points = [kp.strip().lstrip('- •*').strip() for kp in key_points.split('\n') if kp.strip()]
