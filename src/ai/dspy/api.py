...
async def startup_event():
    ...
    try:
        configure_dspy()
        print("[OK] DSPy configured successfully with Google Gemini")
    except Exception as e:
        print(f"[ERROR] Failed to configure DSPy: {e}")
        raise
...
async def generate_quiz(request: QuizRequest):
    ...
    try:
        ...
        # DSPy may return questions as a JSON string, parse if needed
        questions = result.questions
        if isinstance(questions, str):
            # Try to extract JSON array from the string
            # Find the first '[' to last ']' as the JSON array
            start_idx = questions.find('[')
            end_idx = questions.rfind(']')
            if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                json_str = questions[start_idx:end_idx + 1]
                try:
                    questions = json.loads(json_str)
                except json.JSONDecodeError:
                    # If parsing fails, return empty list
                    print(f"Failed to parse quiz questions JSON: {json_str[:200]}")
                    questions = []
            else:
                questions = []
